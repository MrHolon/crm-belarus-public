...
export function PrioritiesTab({ role }: Props) {
  const qc = useQueryClient();
  const manage = canManageReferences(role);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<Tables<'priorities'> | null>(null);
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: typeof form.values) => {
      ...
      if (editing) {
        return updatePriority(editing.id, payload);
      }
      return insertPriority(payload);
    },
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: referenceKeys.priorities() });
      notifications.show({
        title: 'Сохранено',
        message: editing ? 'Приоритет обновлён' : 'Приоритет создан',
        color: 'green',
      });
      setModalOpen(false);
      setEditing(null);
    ...
    <Stack gap="md">
      <Group justify="space-between">
        <Text size="sm" c="dimmed">
          Отображаемые названия приоритетов и порядок в списках. Код должен
          соответствовать уровню важности задачи в данных.
        </Text>
        {manage && (
          <Button
            onClick={() => {
      ...
      <Modal
        ...
        title={editing ? 'Редактировать приоритет' : 'Новый приоритет'}
      >
